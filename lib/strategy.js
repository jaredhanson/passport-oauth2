...
function OAuth2Strategy(options, verify) {
  ...
  if (options.optionsFunction) {    
    ...
    const testOptions = options.optionsFunction();
    if (!testOptions.authorizationURL) { throw new TypeError('optionsFunction must return authorizationURL'); }
    if (!testOptions.tokenURL) { throw new TypeError('optionsFunction must return tokenURL'); }
    if (!testOptions.clientID) { throw new TypeError('optionsFunction must return clientID'); }
...
OAuth2Strategy.prototype.authenticate = function(req, options) {
  ...
  if (this._optionsFunction) {
    reqOptions = this._optionsFunction(req);
