...
OAuth2Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    if (this._state) {
      ...
      var key = this._key;
      if (!req.session[key]) { return this.error(new Error('Failed to find state in session')); }
      var state = req.session[key].state;
      if (!state) { return this.error(new Error('Failed to find state in session')); }
      
      if (state !== req.query.state) {
        return this.fail({ message: 'Cross-site request forgery detected.' }, 403);
      }
    }
