...
OAuth2Strategy.prototype.authenticate = function(req, options) {
  ...
  if (this._optionsFunction) {
    const reqOptions = this._optionsFunction(req);
    if (!reqOptions.authorizationURL) { return this.error(new TypeError('optionsFunction must return authorizationURL')) }
    if (!reqOptions.tokenURL) { return this.error(new TypeError('optionsFunction must return tokenURL')) }
    if (!reqOptions.clientID) { return this.error(new TypeError('optionsFunction must return clientID')) }
