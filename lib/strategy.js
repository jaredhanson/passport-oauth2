 ...
 *   - `optionsFunction`   allows the use of a function for creating options in a multi-tenant environment
 *
 * Examples:
 *
...
function OAuth2Strategy(options, verify) {
  ...
  if (options.optionsFunction) {
    if (typeof options.optionsFunction != 'function') { throw new TypeError('optionsFunction must be a function'); }
    if (!options.sessionKey) { throw new TypeError('OAuth2Strategy requires a session key when using an options function'); }
    this._optionsFunction = options.optionsFunction;
  } else {
...
OAuth2Strategy.prototype.authenticate = function(req, options) {
  ...
  if (this._optionsFunction) {
    this._optionsFunction(req, function(err, reqOptions) {
      if (err) {
        return self.error(err);
      }

      if (!reqOptions.authorizationURL) { return self.error(new TypeError('optionsFunction must return authorizationURL')) }
      if (!reqOptions.tokenURL) { return self.error(new TypeError('optionsFunction must return tokenURL')) }
      if (!reqOptions.clientID) { return self.error(new TypeError('optionsFunction must return clientID')) }
  
      // preserve function for testing, reassign below
      getOAuthAccessTokenFunction = self._oauth2.getOAuthAccessToken;
      self._oauth2 = new OAuth2(reqOptions.clientID, reqOptions.clientSecret,
        '', reqOptions.authorizationURL, reqOptions.tokenURL, );
      self._oauth2.getOAuthAccessToken = getOAuthAccessTokenFunction;

      var meta = {
        authorizationURL: self._oauth2._authorizeUrl,
        tokenURL: self._oauth2._accessTokenUrl,
        clientID: self._oauth2._clientId
      }
      authenticate(req, meta, callbackURL);
    });
  } else {
    var meta = {
  ...
  function authenticate(req, meta, callbackURL) {
      ...
      function loaded(err, ok, state) {
        ...
        self._oauth2.getOAuthAccessToken(code, params,
          function(err, accessToken, refreshToken, params) {
            if (err) { console.log(err); return self.error(self._createOAuthError('Failed to obtain access token', err)); }
