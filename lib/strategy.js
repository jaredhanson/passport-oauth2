...
OAuth2Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    if (this._pkceMethod) {
      var key = this._key;
      var verifier = req.session[key].verifier;
      if (!verifier) {
        return this.fail({ message: 'Unable to code verifier from session.' }, 403);
      }
      
      delete req.session[key].verifier;
      if (Object.keys(req.session[key]).length === 0) {
        delete req.session[key];
      }
    ...
    if (this._pkceMethod) {
      ...
      case 'S256':
        verifier = uid(32);
        var xchallenge = crypto.createHash('sha256').update(verifier, 'ascii').digest();
        console.log(xchallenge);
        var xc = base64url(xchallenge);
        console.log(xc);
        challenge = xc;
        break;
      default: // not enabled
      ...
      var key = this._key;
      if (!req.session[key]) { req.session[key] = {}; }
      req.session[key].verifier = verifier;
